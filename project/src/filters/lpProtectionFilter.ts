...
export class LPProtectionFilter {
  ...
  private async checkKnownLockers(connection: Connection, poolAddress: string): Promise<LPStatus> {
    ...
    try {
      const knownLockerPrograms = [
        'TEAM9mmNNYpkVMqJ1KzgeVtaHvVRN5xTRjAqze9tvydP', // TeamFinance
        'UNICjf5rF2CXeaKv3cYLSHgHHmTq4A7W8GFbYQ8N8Qz',  // Unicrypt (example)
        'PINKSaLE1234567890123456789012345678901234'      // PinkSale (example)
      ];

      for (const lockerProgram of knownLockerPrograms) {
        try {
          const lockerPubkey = new PublicKey(lockerProgram);
