...
async function main() {
  try {
    ...
    logger.info(`📊 Mode: ${config.paperMode ? 'PAPER TRADING' : '🔴 LIVE TRADING'}`);
    ...
    logger.info(`💰 Quote Amount: ${config.quoteAmount} SOL`);
    logger.info(`📈 Take Profit: ${config.takeProfit}%`);
    logger.info(`📉 Stop Loss: ${config.stopLoss}%`);
    logger.info(`⏰ TTL: ${config.ttlMinutes} minutes`);
    logger.info(`🔗 RPC Endpoints: ${config.rpcEndpoints.length}`);
    logger.info(`⚡ Max Concurrent Trades: ${config.maxConcurrentTrades}`);
    logger.info(`🔍 Max Concurrent Filters: ${config.maxConcurrentFilters}`);
    ...
    if (!config.paperMode) {
      ...
      // Add 5 second delay for live mode
